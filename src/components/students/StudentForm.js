...
const StudentForm = ({ fetchStudents, editingStudent, setEditingStudent, onClose }) => {
  ...
  useEffect(() => {
    if (editingStudent) {
      const formattedStudent = {
        ...editingStudent,
        dob: editingStudent.dob.split('T')[0],
        documents: editingStudent.documents || []
      };
      setForm(formattedStudent);
      if (editingStudent.photoUrl) {
        setPreview(editingStudent.photoUrl);
      }
      if (editingStudent.documents) {
        setUploadedDocuments(editingStudent.documents);
      }
    } else {
      resetForm();
    }
  ...
  const validateForm = () => {
    ...
    // Guardian contact validation if provided
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <div className="space-y-4">
                    ...
                    {uploadedDocuments.length > 0 && (
                      <div className="mt-2">
                        <p className="text-sm text-gray-700 mb-2">Previously Uploaded:</p>
                        <ul className="space-y-2">
